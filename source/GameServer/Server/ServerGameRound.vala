...
namespace GameServer
{
    abstract class ServerGameRound
    {
        ...
        {
            GameRoundServerPlayer player = get_server_player(players, player_index);
            ServerMessageDeadWallDraw message = new ServerMessageDeadWallDraw(tile);
            foreach (GameRoundServerPlayer p in players)
            {
                if (settings.reveal_all_tiles == OnOffEnum.ON || p == player || p.server_player.state != ServerPlayer.State.PLAYER || p.server_player.bot || open)
                {
        ...
        public void game_draw(int[] tenpai_indices, int[] nagashi_indices, GameDrawType draw_type, ArrayList<Tile> all_hands)
        {
            ...
            Environment.log(LogType.DEBUG, "ServerGameRound", @"game_draw: Sending ServerMessageDraw to $(players.size) players");
            foreach (GameRoundServerPlayer pl in players)
                pl.server_player.send_message(message);

            finished = true;
            result = new RoundFinishResult.draw(tenpai_indices, nagashi_indices, draw_type);
            Environment.log(LogType.DEBUG, "ServerGameRound", "game_draw: Set finished=true and result");
        }
